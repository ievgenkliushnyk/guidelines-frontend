...
- Пустые ссылки (включая те, на которых будут только JavaScript-обработчики) должны выглядеть так: `<a href="#">Кликни меня</a>`, а не так `<a href="javascript:;">Кликни меня</a>`. Причины описаны в отдельном [сниппете](http://snippets.crisp-studio.com/view/44/pustye-adresa-dlya-ssylok-s-javascript-obrabotchikami).
...
- Для написания плагинов нужно использовать один из [проверенных паттернов](/view/159/patterny-organizacii-koda-v-javascript-pamyatka).
...
### Структура CSS
Для организации CSS используется упрощённая [методология БЭМ](http://bem.github.com/bem-method/pages/beginning/beginning.ru.html). Упрощения состоят в том, что не используются автоматические инструменты, а строгость правил несколько уменьшена. 

#### Термины и определения
*Блок*&nbsp;&mdash; логический элемент (не HTML элемент!) страницы. Примеры блоков: меню, кнопка с иконкой. Внутри блоков могут находится другие блоки (например, кнопка с иконкой внутри меню) и\или элементы.
...
- Разделитель слов в именах блоков и элементов&nbsp;&mdash; символ дефиса, стандартные обозначения блока-обёртки `wrap`, внутреннего блока&nbsp;&mdash; `inner`.
